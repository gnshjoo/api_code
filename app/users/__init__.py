...
def signup():
    ...
    email = request.json['email'] if request.json['email'] else ""
    return jsonify(create_user(email, nickname, name, password, phone))


@mod.route("/detail", methods=["GET"])
def get_user_detail():
    token = request.headers.get("Authorization")
    return jsonify(get_detail(token))
...
def update_password():
    ...
    return jsonify(reset_password(before_password, new_password, token))


@mod.route("/verify/<types>/<code>", methods=["GET"])
def verify_code(types, code):
    return jsonify(verify_code_phone(types, code))
...
def generator_code():
    ...
    return jsonify(generator_code_phone(phone, types))
